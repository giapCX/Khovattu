                ...
                <form action="EditProposalServlet" method="post" class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="proposalType" name="proposalType" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-white">
                            <option value="export" ${proposal.proposalType == 'export' ? 'selected' : ''}>Export</option>
                            <option value="import" ${proposal.proposalType == 'import' ? 'selected' : ''}>Import</option>
                            <c:if test="${role == 'warehouse'}">
                                <option value="repair" ${proposal.proposalType == 'repair' ? 'selected' : ''}>Repair</option>
                            </c:if>
                        </select>
                    </div>
                    <div class="space-y-2">
                        <label for="note" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Note</label>
                        <textarea class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-white" name="note" rows="3">${proposal.note}</textarea>
                    </div> 
                                    ...
                                    <tbody id="itemsBody">
                                        <c:forEach var="detail" items="${proposal.proposalDetails}">
                                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                                ...
                                                <td>
                                                    <input list="materialList" class="w-lg px-2 py-2 border border-gray-300 dark:border-gray-600 rounded-md nameMaterialInput" 
                                                           name="materialName[]" autocomplete="off" value="${detail.materialName}">
                                                ...
                                                <td>
                                                    <input type="text" name="unit[]" class="w-lg px-2 py-2 border border-gray-300 dark:border-gray-600 rounded-md unitMaterial" 
                                                           value="${detail.unit}" readonly>
                                                </td>

                                                <td>
                                                    <input type="number" name="quantity[]" class="w-lg px-2 py-2 border border-gray-300 dark:border-gray-600 rounded-md" 
                                                           value="${detail.quantity}" step="0.01" min="0.01" required>
                                                </td>
                                                ...
                                                <td>
                                                    <select name="materialCondition[]" class="w-3/4 px-2 py-2 border border-gray-300 dark:border-gray-600 rounded-md" required>
                                                        ...
                                                        <option value="damaged" ${detail.materialCondition == 'damaged' ? 'selected' : ''}>Damaged</option>
                                                    </select>
                                                </td>
