         ...
         <body>
             ...
             <script>
                 ...
                 document.addEventListener("DOMContentLoaded", () => {
                     ...
                     // Load suppliers
                     fetch('${pageContext.request.contextPath}/ImportMaterialServlet?getSuppliers=true', {
                         method: 'GET',
                     ...
                     .then(data => {
                         ...
                         document.querySelectorAll(".supplier-select").forEach(select => {
                             ...
                             data.forEach(supplier => {
                                 select.append(new Option(supplier.supplierName, supplier.supplierId));
                             });
                         });
                     })
                     .catch(error => {
                         console.error('Error loading suppliers:', error);
                         alert(`Unable to load supplier data: ${error.message}`);
                     });
                     ...
                     saveButton.addEventListener('click', (e) => {
                         ...
                         console.log('Form data being sent:', Object.fromEntries(formData)); // Log dữ liệu gửi đi
                         fetch('${pageContext.request.contextPath}/ImportMaterialServlet', {
                             method: 'POST',
                             body: formData
                         })
                         .then(response => {
                             console.log('Server response status:', response.status); // Log trạng thái phản hồi
                             if (!response.ok) {
                         ...
                         .then(data => {
                             console.log('Server response data:', data); // Log dữ liệu phản hồi
                             ...
                             if (data.status === 'success') {
                                 ...
                                 document.getElementById('materialTableBody').innerHTML = `
                                     <tr>
                                         ...
                                         <td><input type="text" class="form-control material-code-select" readonly></td>
                                         <td><select class="form-select supplier-select" name="supplierId[]" required><option value="">-- Select Supplier --</option></select></td>
                                         <td><input type="number" class="form-control quantity" name="quantity[]" min="0.01" step="0.01" required></td>
                                         <td><input type="text" class="form-control unit-display" readonly></td>
                                         <td><input type="number" class="form-control unit-price" name="price_per_unit[]" min="0.01" step="0.01" required></td>
                                         <td class="total-price">0.00</td>
                                 ...
                                 errorAlert.textContent = data.message || 'Unknown error from server';
                         ...
                         .catch(error => {
                             ...
                             const errorAlert = document.getElementById('errorAlert');
                             errorAlert.textContent = `An unexpected error occurred: ${error.message}. Please try again or contact support.`;
                             errorAlert.style.display = 'block';
                             setTimeout(() => errorAlert.style.display = 'none', 5000);
                         });
                     });
