            ...
            <div class="max-w-6xl mx-auto">
                <div class="flex justify-between items-center mb-6">
                    <div class="flex items-center gap-4">
                        ...
                        <h2 class="text-2xl font-bold text-gray-800 dark:text-white">List Requests To Be Execute</h2>
                    </div>
                </div>
                ...
                <form action="ListProposalExecute" method="get" class="mb-6 flex flex-wrap gap-4 items-center">
                    <div class="flex-1 min-w-[200px]">
                        <input type="text" name="searchSender" placeholder="Search name sender/note of sender " value="${param.searchSender}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-white">
                    </div>
                    <div class="flex-1 min-w-[200px]">
                        ...
                        <table class="w-full table-auto">
                            <thead>
                                <tr class="bg-primary-600 text-white">
                                    ...
                                    <th class="p-4 text-left">Type of requests</th>
                                    <th class="p-4 text-left">Sender</th>
                                    <th class="p-4 text-left">Note Of Sender</th>
                ...
                <div class="pagination flex items-center justify-center space-x-1">
                    ...
                    <c:choose>
                        <c:when test="${currentPage > 1}">
                            <a href="ListProposalExecute?page=${currentPage - 1}&recordsPerPage=${param.recordsPerPage}&searchType=${param.searchType}&searchSender=${param.searchSender}" class="px-3 py-1 rounded bg-gray-300 hover:bg-gray-400">&lt;</a>
                        </c:when>
                        <c:otherwise>
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a href="ListProposalExecute?page=1&recordsPerPage=${param.recordsPerPage}&searchType=${param.searchType}&searchSender=${param.searchSender}" class="px-3 py-1 rounded border hover:border-blue-500">1</a>
                        </c:otherwise>
                    </c:choose>
                    ...
                    <c:forEach var="i" begin="${currentPage - 1 > 1 ? currentPage - 1 : 2}" end="${currentPage + 1 < totalPages ? currentPage + 1 : totalPages - 1}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a href="ListProposalExecute?page=${i}&recordsPerPage=${param.recordsPerPage}&searchType=${param.searchType}&searchSender=${param.searchSender}" class="px-3 py-1 rounded border hover:border-blue-500">${i}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    ...
                    <c:if test="${totalPages > 1}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a href="ListProposalExecute?page=${totalPages}&recordsPerPage=${param.recordsPerPage}&searchType=${param.searchType}&searchSender=${param.searchSender}" class="px-3 py-1 rounded border hover:border-blue-500">${totalPages}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:if>
                    <!-- Nút trang sau -->
