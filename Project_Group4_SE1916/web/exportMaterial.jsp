...
<html lang="vi">
    ...
    <body>
        <%
            ...
            List<MaterialCategory> categories = cateDAO.getAllChildCategories();
        %>
        ...
        <div class="container">
            ...
            <form action="${pageContext.request.contextPath}/exportMaterial" method="post" class="needs-validation" novalidate>
                <div class="mb-3">
                    <label for="userFN" class="form-label">Người lập phiếu</label>
                    <input type="text" name="userFN" class="form-control" value="${sessionScope.userFullName != null ? sessionScope.userFullName : 'Chưa xác định'}" readonly>
                ...
                <button type="button" class="btn btn-success" onclick="window.print()">In Phiếu</button>
        ...
        <script>
            let materialData = []; // Khởi tạo rỗng

            // Function to attach event listeners to category select
            function attachCategoryListener(categorySelect) {
                categorySelect.addEventListener("change", function () {
                    const selectedCategoryId = this.value;
                    console.log('Selected categoryId:', selectedCategoryId); // Log để kiểm tra
                    ...
                    // Reset
                    nameSelect.innerHTML = '<option value="">-- Chọn tên vật tư --</option>';
                    codeSelect.innerHTML = '<option value="">-- Chọn mã vật tư --</option>';
                    unitInput.value = '';
                    ...
                    //const filteredMaterials = materialData.filter(mat => mat.category_id == selectedCategoryId);
                    const filteredMaterials = materialData.filter(mat => mat.category_id == selectedCategoryId);
                    console.log('Filtered materials:', filteredMaterials); // Log để kiểm tra
                    ...
                    if (filteredMaterials.length > 0) {
                        ...
                        filteredMaterials.forEach(mat => {
                            const opt1 = new Option(mat.name, mat.name);
                            const opt2 = new Option(mat.code, mat.code);
                            nameSelect.add(opt1);
                            codeSelect.add(opt2);
                        });
                        ...
                        showAlert('warning', 'Không có vật tư nào thuộc danh mục này.');
                    }
                });
            }
            ...
            function addMaterialRow() {
                ...
                if (!tableBody) {
                    ...
                    showAlert('danger', 'Không thể thêm vật tư: Lỗi hệ thống.');
                    return;
                }
            ...
            %>
                ...
                attachCategoryListener(row.querySelector('.category-select'));
                updateSerialNumbers();
                updateRemoveButtons();
            ...
            function removeRow(button) {
                ...
                if (!row) {
                    showAlert('danger', 'Không thể xóa dòng: Lỗi hệ thống.');
                    return;
                }
                row.remove();
                updateSerialNumbers();
            ...
            function validateForm() {
                ...
                if (!exportId) {
                    showAlert('danger', 'Mã xuất không được để trống.');
                    return false;
                }
                if (!voucherId) {
                    showAlert('danger', 'Mã phiếu không được để trống.');
                    return false;
                }
                if (!purpose) {
                    showAlert('danger', 'Lý do xuất kho không được để trống.');
                    return false;
                }

                // Validate material rows
                if (materialNames.length === 0) {
                    showAlert('danger', 'Phải có ít nhất một vật tư.');
                    return false;
                }

                for (let i = 0; i < materialNames.length; i++) {
                    if (!materialNames[i].value.trim()) {
                        showAlert('danger', `Tên vật tư không được để trống tại dòng ${i + 1}`);
                        rows[i].classList.add('error-row');
                        return false;
                    }
                    if (!materialCodes[i].value.trim()) {
                        showAlert('danger', `Mã vật tư không được để trống tại dòng ${i + 1}`);
                        rows[i].classList.add('error-row');
                        return false;
                    }
                    if (!quantities[i].value || quantities[i].value <= 0) {
                        showAlert('danger', `Số lượng phải lớn hơn 0 tại dòng ${i + 1}`);
                        rows[i].classList.add('error-row');
                        return false;
                    }
                    if (!categories[i].value) {
                        showAlert('danger', `Loại vật tư không được để trống tại dòng ${i + 1}`);
                        rows[i].classList.add('error-row');
                        return false;
                    }
                    if (!units[i].value) {
                        showAlert('danger', `Đơn vị không được để trống tại dòng ${i + 1}`);
                        rows[i].classList.add('error-row');
                        return false;
                    }
                    if (!conditions[i].value) {
                        showAlert('danger', `Điều kiện không được để trống tại dòng ${i + 1}`);
                        rows[i].classList.add('error-row');
                        return false;
                    }
                }
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                if (!tableBody || !addButton) {
                    ...
                    showAlert('danger', 'Lỗi hệ thống: Không tìm thấy bảng hoặc nút thêm.');
                    return;
                }
                ...
                fetch('${pageContext.request.contextPath}/exportMaterial', {
                    ...
                    headers: {
                        ...
                        .then(response => {
                            if (!response.ok) {
                                ...
                                return response.text().then(errorText => {
                                    throw new Error(`Lỗi từ server: ${response.status} - ${errorText || 'Không có thông tin lỗi'}`);
                                });
                            }
                        ...
                        .then(data => {
                            materialData = data;
                            console.log('Material data loaded:', materialData); // Log dữ liệu để debug
                            document.querySelectorAll(".category-select").forEach(attachCategoryListener);
                        })
                        .catch(error => {
                            console.error('Error fetching materials:', error);
                            // Hiển thị lỗi chi tiết cho người dùng
                            showAlert('danger', `Không thể tải dữ liệu vật tư. Lỗi: ${error.message}`);
                        });
