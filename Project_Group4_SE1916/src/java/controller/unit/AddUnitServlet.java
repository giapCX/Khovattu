...
public class AddUnitServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBContext.getConnection()) {
            ...
            boolean exists = unitDAO.checkUnitExistsByName(name);
            if (exists) {
                request.setAttribute("error", "Unit name already exists!");
                request.getRequestDispatcher("addUnit.jsp").forward(request, response);
                return;
            } else {
                boolean inserted = unitDAO.addUnit(name);
                if (inserted) {
                    request.setAttribute("success", "Unit added successfully.");
                } else {
                    request.setAttribute("error", "Failed to add unit.");
                }
            }
            request.getRequestDispatcher("addUnit.jsp").forward(request, response);
